...
int main(void)
{
    ...
    while(cases--)
    {
        Punto ini, fin, topLeft, topRight, bottomLeft, bottomRight;
        Linea seg, arriba, derecha, izquierda, abajo;
        vector <int> puntos(8);
        for(int i = 0 ; i < 8 ; i++)
            cin >> puntos[i];
        ini.x = puntos[0];
        ...
        topRight.x = puntos[6];
        topRight.y = puntos[5];
        bottomLeft.x = puntos[4];
        bottomLeft.y = puntos[7];
        bottomRight.x = puntos[6];
        bottomRight.y = puntos[7];
        seg = Linea(ini, fin);
        arriba = Linea(topLeft, topRight);
        abajo = Linea(bottomLeft, bottomRight);
        derecha = Linea(topRight, bottomRight);
        izquierda = Linea(topLeft, bottomLeft);
        //5 6 5 6 4 7 7 5
        //3 3 2 2 1 4 6 1
        //Saber si esta dentro del rectangulo
        // Prueba de punto en poligono convexo.
        // En el perimetro = -1, Fuera = 0, Dentro = 1.
        bool dentro = false, up = false, left = false, down = false, right = false;
        if((ini.y >= bottomRight.y && ini.y <= topRight.y && ini.x >= bottomLeft.x && ini.x <= bottomRight.x) && (fin.y >= bottomRight.y && fin.y <= topRight.y && fin.x >= bottomLeft.x && fin.x <= bottomRight.x))
            dentro = true;
        up = InterseccionSegmentos(seg, arriba);
        down = InterseccionSegmentos(seg, abajo);
        left = InterseccionSegmentos(seg, izquierda);
        right = InterseccionSegmentos(seg, derecha);
        Double x0, y0, ancho, altura;
        x0 = min(puntos[4], puntos[6]);
        y0 = min(puntos[5], puntos[7]);
        ancho = abs(puntos[4] - puntos[6]);
        altura = abs(puntos[5] - puntos[7]);
        if(up == -1 || down == -1 || left == -1 || right == -1)
            cout << "T\n";
        else if(up || down || left || right)
            cout << "T\n";
        else{
            if((x0 <= puntos[0] && puntos[0] <= x0 + ancho && y0 <= puntos[1] && puntos[1] <= y0 + altura) || (x0 <= puntos[2] && puntos[2] <= x0 + ancho && y0 <= puntos[3] && puntos[3] <= y0 + altura))
                cout << "T\n";
            else
                cout << "F\n";
        }
